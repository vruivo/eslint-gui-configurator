...
module.exports = function generateRuleControls(schema, sch_nr, rule_name) {
  ...
  var html = '';
  html += '<div class="hidden" style="overflow-wrap:break-word; width:1000px; color:green;">'+schema_str + '</div>'+'<br>';  // debug line
  schema_str = schema_str.replace(/"/g, '%22');  // url encode "
  html += '<div class="controls" id="'+sch_nr+'_rule" data-schema="'+schema_str+'" style="min-height:5px; margin-top: 3px;">';
  // text box with rule configuration
  html += '<input id="'+sch_nr+'_text" name="'+rule_name+'" type="text" width="100%" size="60"' +
    'class="rule-text" readonly="true" onchange="updateRule(this)" style="margin-bottom:0.5em">';
